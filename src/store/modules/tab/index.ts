...
export const useTabStore = defineStore('tab', {
  ...
  actions: {
    ...
    removeOther(curPath: string) {
      this.setTabs(this.tabs.filter((item) => item.path === curPath || item.affix))
    ...
    removeLeft(curPath: string) {
      ...
      const filterTabs = this.tabs.filter((item, index) => index >= curIndex || item.affix)
    ...
    removeRight(curPath: string) {
      const curIndex = this.tabs.findIndex((item) => item.path === curPath)
      const filterTabs = this.tabs.filter((item, index) => {
        return index <= curIndex || item.affix
      })
      this.setTabs(filterTabs)
      if (!filterTabs.find((item) => item.path === this.activeTab)) router.push(filterTabs[filterTabs.length - 1].path)
    },
